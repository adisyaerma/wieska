...
@section('jenisTiketActive', 'active')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card mt-1">
            <div class="card-header d-flex justify-content-between align-items-center py-5">
                <h5 class="mb-0 fs-4">Jenis Tiket</h5>
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                        data-bs-target="#tambahJenisTiket">
                        Tambah
                    </button>
                </div>
            </div>
            ...
            <!-- Modal -->
            <div class="modal fade" id="tambahJenisTiket" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog">
                    <form action="{{ route('jenis_tiket.store') }}" method="POST" class="modal-content">
                        @csrf
            ...
            <div class="table-responsive text-nowrap">
                <table class="table" id="kategoriTable">
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($jenis_tikets as $i => $jenis_tiket)
                            <tr>
                                <td><span>{{ $i + 1 }}</span></td>
                                <td>{{ $jenis_tiket->jenis_tiket }}</td>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <button type="button" class="dropdown-item" href="javascript:void(0);"
                                                data-bs-toggle="modal" data-bs-target="#editJenisTiket{{ $jenis_tiket->id }}"><i
                                                    class="icon-base bx bx-edit-alt me-1"></i>
                                                Edit</button>
                                            <form class="form-hapus" action="{{ route('jenis_tiket.destroy', $jenis_tiket->id) }}" method="POST">
                                                @csrf 
                                                @method('DELETE')
        ...
        @foreach($jenis_tikets as $jenis_tiket)
            <div class="modal fade" id="editJenisTiket{{ $jenis_tiket->id }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog">
                    <form action="{{ route('jenis_tiket.update', $jenis_tiket->id) }}" method="POST" class="modal-content">
                        @csrf @method('PUT')
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label">Jenis Tiket</label>
                                <input type="text" name="jenis_tiket" class="form-control"
                                    value="{{ $jenis_tiket->jenis_tiket }}" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Harga</label>
                                <input type="text" name="harga" class="form-control" value="{{ $jenis_tiket->harga }}" required>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-label-secondary" data-bs-dismiss="modal">Batal</button>
