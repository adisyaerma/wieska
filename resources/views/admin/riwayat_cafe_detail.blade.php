...
@section('title', 'Detail Riwayat Cafe')
  ...
  <div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card mt-1">
      <div class="card-header d-flex justify-content-between align-items-center py-5">
        ...
        <div>
          <h5 class="mb-0">Kasir: {{ optional($cafe->karyawan)->nama ?? '—' }}</h5>
          <h5 class="mb-0">Pelanggan: {{ $cafe->nama_pelanggan }}</h5>
        </div>
      ...
      <div class="table-responsive text-nowrap">
        <table class="table" id="barangTable">
          ...
          <tbody>
            @foreach($cafe->details as $i => $d)
              <tr>
                <td>{{ $i + 1 }}</td>
                {{-- ambil nama barang dari tabel barang --}}
                <td>{{ $d->menu->stokBarang->nama_barang ?? '-' }}</td>
                {{-- ambil satuan dari tabel stok_barang --}}
                <td>{{ $d->menu->stokBarang->satuan->satuan ?? '-' }}</td>
                <td>{{ $d->jumlah }}</td>
                <td>Rp {{ number_format($d->menu->harga_jual, 0, ',', '.') }}</td>
                  ...
                  <div class="dropdown">
                    ...
                    <div class="dropdown-menu">
                      ...
                      <button type="button" class="dropdown-item" data-bs-toggle="modal"
                        data-bs-target="#editBarang{{ $d->id }}">
                        <i class="icon-base bx bx-edit-alt me-1"></i> Edit
                      </button>

                      <!-- Tombol Hapus -->
                      <form class="form-hapus" action="{{ route('riwayat_cafe.details.destroy', $d->id) }}" method="POST">
                        @csrf @method('DELETE')
    ...
    @foreach($cafe->details as $detail)
      <div class="modal fade" id="editBarang{{ $detail->id }}" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <form action="{{ route('riwayat_cafe.details.update', $detail->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title">Edit Detail Pesanan</h5>
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <select class="form-select" name="menu_id" required>
                    ...
                    @foreach($menus as $menu)
                      @php
                        $namaBarang = optional($menu->stokBarang)->nama_barang ?? 'Tidak ada';
                        $satuan = optional($menu->stokBarang)->satuan ?? '-';
                      @endphp
                ...
                <div class="mb-3">
                  ...
                  <input type="number" class="form-control" name="jumlah" value="{{ $detail->jumlah }}" min="1" required>
                </div>
              </div>
